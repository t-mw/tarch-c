...
void sokol_state_init(struct SokolState* state)
{
  struct sg_pipeline pipeline = sg_make_pipeline(&(sg_pipeline_desc){
      ...
      .layout = { .buffers[0] = sshape_buffer_layout_desc(),
                  .attrs = { [ATTR_basic_vs_position] = sshape_position_attr_desc(),
                             [ATTR_basic_vs_color0] = sshape_color_attr_desc() } },
...
void sokol_state_draw(struct SokolState* state)
{
  ...
  sx_mat4 proj =
      sx_mat4_perspectiveFOV(SX_PI / 3.0f, sapp_widthf() / sapp_heightf(), 0.01f, 10.0f, true);
  sx_mat4 view = sx_mat4_view_lookat(sx_vec3f(0.0f, 1.5f, 6.0f), sx_vec3f(0.0f, 0.0f, 0.0f),
                                     sx_vec3f(0.0f, 1.0f, 0.0f));
  ...
  sg_apply_bindings(
      &(sg_bindings){ .vertex_buffers[0] = state->vbuf, .index_buffer = state->ibuf });
